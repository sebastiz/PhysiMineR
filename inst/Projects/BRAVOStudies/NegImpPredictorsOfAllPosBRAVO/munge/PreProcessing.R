...
BravoAndHRM <- HRMImportMainTwo %>% inner_join(AllBravo, by ="HospNum_Id") %>%
...
BravoAndHRM<-GORD_AcidBRAVO(BravoAndHRM)



ImpAndHRM <- BravoAndHRM %>% inner_join(ImpAll, by ="HospNum_Id") %>%
  group_by(HospNum_Id) %>%
  slice(1)
...
DiagTwo$Dx<-str_extract_all(DiagTwo$WholeReport, paste0("upragas|neffecti|ackham|utcrack|[Aa]peris|[Ff]requent [Ff]ail" , simplify = FALSE))
DiagTwo$Dx <- sapply(DiagTwo$Dx, toString)
...
# #Univariate analysis:
# NegImpPosBRAVOFinal<-BravoAndHRMNoMissing[BravoAndHRMNoMissing$AcidRefluxBRAVO==1,]
# NegImpNegBRAVOFinal<-BravoAndHRMNoMissing[BravoAndHRMNoMissing$AcidRefluxBRAVO==0,]
#
# #Univariate analysis
