...
Impedance2_Clean<-dataImpClean(Impedance2)
...
#This gives the matching hospital numbers
mydataset<-data.frame(intersect(AllBravo$HospNum_Id, ImpAll$HospNum_Id))
names(mydataset)<-"HospNum_Id"

#Now need to extract all the BRAVO which have an impedance ie all the BRAVO for hospital numbers with impedance
MyBravoFromImpedance<-data.frame(AllBravo[which(AllBravo$HospNum_Id %in% mydataset$HospNum_Id),])


mydatasetBRAVO<-which(AllBravo$HospNum_Id,arr.ind  = TRUE)
