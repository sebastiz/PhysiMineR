...
ImpAll<-dataImpSymptoms(ImpAll) 


#Extract who has reflux based on Demeester or on the Number of refluxes from both Impedance and BRAVO
dataImpSypmAndImpedanceMain<-GORD_AcidImp(Impedance2_Clean) 



#Mock data to be cleaned up:
AllBravo$ReflDay1NumberofRefluxesSupine<-sample(100, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay1NumberofRefluxesTotal<-sample(100, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay3NumberofRefluxesUpright<-sample(40, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay3NumberofRefluxesSupine<-sample(40, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay3FractionTimepHLessThan4Supine<-sample(100, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay3FractionTimepHLessThan4Upright<-sample(40, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay3FractionTimepHLessThan4Total<-sample(40, size = nrow(AllBravo), replace = TRUE)

AllBravo$ReflDay4NumberofRefluxesUpright<-sample(40, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay4NumberofRefluxesTotal<-sample(40, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay4NumberofRefluxesSupine<-sample(40, size = nrow(AllBravo), replace = TRUE)

AllBravo$ReflDay4FractionTimepHLessThan4Supine<-sample(100, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay4FractionTimepHLessThan4Upright<-sample(40, size = nrow(AllBravo), replace = TRUE)
AllBravo$ReflDay4FractionTimepHLessThan4Total<-sample(40, size = nrow(AllBravo), replace = TRUE)

#Need to get those extra fields in for the database before you can do this step.
...
#Now need to extract all the BRAVO which have an impedance ie all the BRAVO for hospital numbers with impedance
MyBravoFromImpedance<-data.frame(AllBravo[which(AllBravo$HospNum_Id %in% NegativeImpedance$HospNum_Id),])
