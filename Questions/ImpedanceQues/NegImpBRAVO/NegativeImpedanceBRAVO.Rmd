...
NegImpPOS_BRAVO_WithImpAndHRMNoMissing<-missing[!grepl("Bad|Remove",missing$group),]

#Pick the columns with good data in them from this set
NegImpPOS_BRAVO_WithImpAndHRMNoMissing<-NegImpPOS_BRAVO_WithImpAndHRM[,match(as.character(NegImpPOS_BRAVO_WithImpAndHRMNoMissing$feature),names(NegImpPOS_BRAVO_WithImpAndHRM))]
                                                                               
                                                                               
#2. Get some basic descriptive stats:
...
NegImpPOS_BRAVO_WithImpAndHRMNoMissingNums<-dplyr::select_if(NegImpPOS_BRAVO_WithImpAndHRMNoMissing, is.numeric)
rr<-describe(NegImpPOS_BRAVO_WithImpAndHRMNoMissing)



#Get the correlations
library(Hmisc)
correlations <- rcorr(as.matrix(NegImpPOS_BRAVO_WithImpAndHRMNoMissingNums))
#Look at the correlations:

#corrgram(NegImpPOS_BRAVO_WithImpAndHRMNoMissingNums)


#Only pick the very correlated functions:
A <- correlations$r    # matrix A here contains the correlation coefficients
B <- correlations$P   # matrix B here contains the corresponding p-values
sig_element = ifelse((A > 0.3) , B, NA)
library(corrgram)
corrgram(sig_element)

#Plot the continuous variables to see how they are distributed as a bar plot
#plot_histogram(NegImpPOS_BRAVO_WithImpAndHRMNoMissing)
